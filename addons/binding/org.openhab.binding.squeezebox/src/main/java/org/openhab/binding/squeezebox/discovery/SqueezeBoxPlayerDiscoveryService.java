 ...
 *
 */
public class SqueezeBoxPlayerDiscoveryService extends AbstractDiscoveryService
		implements SqueezeBoxPlayerEventListener {
	private final Logger logger = LoggerFactory
			.getLogger(SqueezeBoxPlayerDiscoveryService.class);
	...
	public void playerAdded(SqueezeBoxPlayer player) {
		...
		DiscoveryResult discoveryResult = DiscoveryResultBuilder
				...
				.withLabel("SqueezeBox Player " + player.getName()).build();
	...
	private void setupRequestPlayerJob() {
		...
		requestPlayerJob = scheduler.scheduleAtFixedRate(runnable, 10, 60,
				TimeUnit.SECONDS);
	}
